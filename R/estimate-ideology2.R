...
estimateIdeology2 <- function(user, friends, verbose=TRUE, exact=FALSE, 
  ...
  y <- matrix((tweetscoresmod::refdataCA$id %in% friends)*1, nrow=1)
  # info message
  if (sum(y)==0){
  ...
  message(user, " follows ", sum(y), " elites: ",
      paste(tweetscoresmod::refdataCA$colname[
        tweetscoresmod::refdataCA$id %in% friends], collapse=", "))
  # estimation
  values <- supplementaryRows(tweetscoresmod::refdataCA, y)
  # normalizing
  theta <- tweetscoresmod::refdataCA$qs$theta[which.min(abs(values[1] - (tweetscoresmod::refdataCA$qs$value)))]
  # adding random noise
  ...
  if (replace_outliers && (theta == -Inf || theta == Inf)){
    ...
    if (theta == -Inf){ theta <- rs[rs<tweetscoresmod::refdataCA$qs$theta[2]][1] }
    if (theta == Inf){ theta <- rs[rs>tweetscoresmod::refdataCA$qs$theta[100]][1] }
  }

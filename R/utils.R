.onAttach <- function(...) {
  ...
  packageStartupMessage("##\n## tweetscoresmod: tools for the analysis of Twitter data")
  packageStartupMessage("## Pablo Barbera (USC)")
  packageStartupMessage("## www.tweetscoresmod.com\n##")
...
metropolis.logit <- function(y,
                             alpha.i=tweetscoresmod::posterior_samples$alpha,
                             gamma.i=tweetscoresmod::posterior_samples$gamma,
                             phi.i=tweetscoresmod::posterior_samples$phi,
                             beta.init=rep(log(sum(y)), chains),
                             theta.init=rnorm(chains, 0, 1),
                             mu_beta.i=tweetscoresmod::posterior_samples$mu_beta,
                             sigma_beta.i=tweetscoresmod::posterior_samples$sigma_beta,
...
ml.logit <- function(y,
                     alpha=colMeans(tweetscoresmod::posterior_samples$alpha),
                     gamma=mean(tweetscoresmod::posterior_samples$gamma),
                     phi=colMeans(tweetscoresmod::posterior_samples$phi),
                     beta.init=log(sum(y)),
                     theta.init=rnorm(1, 0, 1),
                     mu_beta=mean(tweetscoresmod::posterior_samples$mu_beta),
                     sigma_beta=mean(tweetscoresmod::posterior_samples$sigma_beta),

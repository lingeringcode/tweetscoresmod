...
estimateIdeology <- function(user, friends, verbose=TRUE, method="MCMC",
  ...
  y <- tweetscoresmod::posterior_samples$id %in% friends
  # info message
  if (sum(y)==0){
    # Mod original: stop() to print() warning
    print("User follows 0 elites!")
    print("Trying estimateIdeology2.")
    results <- "zero_elites"
    return(results)
  }
  message(user, " follows ", sum(y), " elites: ",
      paste(tweetscoresmod::posterior_samples$screen_name[
          tweetscoresmod::posterior_samples$id %in% friends ], collapse=", "))
  # estimation
  if (method=="MCMC"){
